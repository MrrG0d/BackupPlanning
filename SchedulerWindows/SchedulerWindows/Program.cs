        ...
        static void Main(string[] args)
        {
            ...
            if (int.Parse(data[5]) < 5)
            {
                string commands = $@"D:
                                 ...
                                 mysqldump -uroot -pmysql --lock-all-tables {data[2]} > {data[3]}\{args[0]}-{int.Parse(data[5]) + 1}.sql
                                 mysqlcheck -u root -p --auto-repair {data[2]}";
            ...
            else
            {
                ...
                string commands = $@"D:
                                 cd D:\Ampps\mysql\bin
                                 mysqldump -uroot -pmysql --lock-all-tables {data[2]} > {data[3]}\{args[0]}-5.sql
                                 mysqlcheck -u root -p --auto-repair {data[2]}";
